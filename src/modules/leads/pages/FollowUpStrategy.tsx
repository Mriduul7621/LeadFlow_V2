      ...
      <div className="flex items-center gap-4 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-slate-400 mt-1 flex items-center gap-2">
            ...
            {item.assignedEmployeeName || item.assignedTo || t('unassigned')}
          </p>
      ...
      <div className="text-right shrink-0">
        ...
        {item.dueState === 'overdue' && (
          <span className="inline-flex items-center gap-1 mt-2 px-2 py-0.5 bg-red-50 text-red-600 text-[9px] font-black uppercase tracking-widest">
            ...
            {t('overdue')} {item.overdueDays ? `${item.overdueDays}d` : ''}
          </span>
        )}
        <p className="text-[10px] text-[#978C21] mt-2 inline-flex items-center gap-1 font-medium">
          {t('lead360')} <ChevronRight className="w-3 h-3" />
        </p>
      </div>
    </button>
  );
...
export default function FollowUpStrategy() {
  ...
  const load = useCallback(async (bucket: Bucket) => {
    ...
    try {
      ...
      toast.error(err?.message || t('followUpQueueLoadError'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const tabs: Array<{ id: Bucket; label: string; icon: typeof Clock; count: number }> = [
    { id: 'overdue', label: t('followUpBucketOverdue'), icon: AlertTriangle, count: counts.overdue },
    { id: 'today', label: t('followUpBucketToday'), icon: Clock, count: counts.today },
    { id: 'upcoming', label: t('followUpBucketUpcoming'), icon: Calendar, count: counts.upcoming },
  ];
  const bucketLabel = (bucket: string): string => {
    switch (bucket) {
      case 'overdue': return t('followUpBucketOverdue');
      case 'today': return t('followUpBucketToday');
      case 'upcoming': return t('followUpBucketUpcoming');
    ...
    <div className="space-y-8 pb-24 bg-white font-sans">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-8">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-800">{t('followUpQueueTitle')}</h1>
            <p className="text-slate-400 text-[10px] uppercase tracking-[0.25em] mt-2 italic">
              {t('followUpQueueSubtitle')} · {data?.todayDate || '—'}
            </p>
          </div>
        </div>
        <button
        ...
        <div className="space-y-3">
          {(data?.items || []).length === 0 ? (
            <div className="text-center py-16 text-slate-400 text-[11px] uppercase tracking-widest border border-dashed border-slate-200">
              {t('noFollowUpsInVisibility', { bucket: bucketLabel(active) })}
            </div>
          ) : (
            (data?.items || []).map(item => (
              <div key={item.id}>
