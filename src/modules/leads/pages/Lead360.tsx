...
export default function Lead360() {
  ...
  const handleCompleteScheduled = async (saId: string) => {
    try {
      ...
      toast.success('Activity marked completed — history created.');
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-sm border border-slate-100 shadow-sm p-6">
        ...
        <p className="text-[11px] text-slate-400 mb-4">Server-authoritative calendar (Asia/Dhaka, visibility-enforced) — calls, meetings, follow-ups and tasks tied to this lead. Stored in <code className="px-1 py-0.5 bg-slate-100 rounded text-[10px]">scheduled_activities</code>.</p>
        <form onSubmit={handleSchedule} className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-6 bg-[#FBFAF8] border border-slate-100 p-4 rounded-sm">
          ...
          <input type="text" placeholder="Meeting type (optional)" value={schedForm.meetingType} onChange={e => setSchedForm({ ...schedForm, meetingType: e.target.value })} className="border border-slate-200 rounded-sm px-2 py-2 text-xs" />
          <input type="text" placeholder="Location (optional)" value={schedForm.location} onChange={e => setSchedForm({ ...schedForm, location: e.target.value })} className="border border-slate-200 rounded-sm px-2 py-2 text-xs" />
          ...
          <button type="submit" disabled={schedSaving} className="bg-[#978C21] text-white text-xs font-bold px-3 py-2 rounded-sm disabled:opacity-50 md:col-span-2">Schedule</button>
        </form>
        {scheduled.length === 0 ? (
                ...
                {isEditing ? (
                  <div className="space-y-2">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      <input type="text" value={editForm.title} onChange={e=> setEditForm({...editForm, title:e.target.value})} placeholder="Title" className="border border-slate-200 rounded-sm px-2 py-1.5 text-xs" />
                      ...
                      <input type="number" value={editForm.durationMinutes} onChange={e=> setEditForm({...editForm, durationMinutes:e.target.value})} placeholder="Duration" className="border border-slate-200 rounded-sm px-2 py-1.5 text-xs" min={1} max={1440} />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      ...
                      <input type="text" value={editForm.meetingType} onChange={e=> setEditForm({...editForm, meetingType:e.target.value})} placeholder="Meeting type" className="border border-slate-200 rounded-sm px-2 py-1.5 text-xs" />
                      <input type="text" value={editForm.location} onChange={e=> setEditForm({...editForm, location:e.target.value})} placeholder="Location" className="border border-slate-200 rounded-sm px-2 py-1.5 text-xs" />
                    </div>
                    <input type="text" value={editForm.remarks} onChange={e=> setEditForm({...editForm, remarks:e.target.value})} placeholder="Remarks" className="w-full border border-slate-200 rounded-sm px-2 py-1.5 text-xs" />
                    <div className="flex gap-2 justify-end">
                      <button onClick={()=> setEditingId(null)} className="text-[11px] font-bold text-slate-500 border border-slate-200 px-3 py-1 rounded-sm">Cancel</button>
                      <button onClick={()=> handleUpdateScheduled(sa.id)} className="text-[11px] font-bold text-white bg-[#978C21] px-3 py-1 rounded-sm">Save</button>
                    </div>
                  </div>
                ) : (
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-slate-800 truncate">{sa.title || (sa.activityType === 'call' ? 'Call' : sa.activityType === 'meeting' ? 'Meeting' : sa.activityType === 'task' ? 'Task' : 'Follow-up')} · {sa.activityType} {(sa as any).priority ? `· ${(sa as any).priority}` : ''}</p>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 flex-wrap justify-end">
                    {isScheduled ? (
                      <>
                        <button onClick={() => handleCompleteScheduled(sa.id)} className="text-[11px] font-bold text-emerald-600 hover:text-emerald-800 border border-emerald-100 px-2 py-1 rounded-sm">Complete</button>
                        <button onClick={() => handleCancelScheduled(sa.id)} className="text-[11px] font-bold text-amber-600 hover:text-amber-800 border border-amber-100 px-2 py-1 rounded-sm">Cancel</button>
                        <button onClick={() => startEditScheduled(sa)} className="text-[11px] font-bold text-slate-600 hover:text-slate-800 border border-slate-200 px-2 py-1 rounded-sm">Edit</button>
                      ...
                      <>
                        ...
                        <button disabled className="text-[11px] font-bold text-slate-300 border border-slate-100 px-2 py-1 rounded-sm cursor-not-allowed">Edit</button>
                      </>
                    )}
                  </div>
                </div>
