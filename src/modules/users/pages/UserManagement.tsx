...
import { adminService, ensureFeaturePermissions } from '../../admin/services/adminService';
...
import EnterpriseAccessPanel from '../components/EnterpriseAccessPanel';
...
export default function UserManagement() {
  ...
  const [activeStep, setActiveStep] = useState<1 | 2 | 3 | 4 | 5>(3);
    ...
    <div className="space-y-10 max-w-7xl mx-auto pb-16">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex items-center gap-1 border border-slate-200/60 p-1.5 rounded-sm bg-[#FBFAF8] shadow-sm">
          {[1, 2, 3, 4, 5].map((step) => {
            const label = step === 1 ? '1. Depts' : step === 2 ? '2. Roles' : step === 3 ? '3. Roster' : step === 4 ? '4. Hierarchy' : '5. Access & Audit';
            return (
              <button
                key={step}
        ...
        <div className="min-h-[400px]">
          ...
          {activeStep === 5 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <EnterpriseAccessPanel users={users} />
            </motion.div>
          )}
        </div>
      )}
