...
export default function EnterpriseAccessPanel({ users }: { users: User[] }) {
  ...
  const saveOverrides = async () => {
    ...
    if (!response.ok) {
      toast.error('Permission overrides could not be saved.');
      return;
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      <section className="lg:col-span-7 bg-[#FBFAF8] border border-slate-200 p-6 rounded-sm space-y-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-900">Effective Permission Overrides</h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Role permission plus individual user override.</p>
          </div>
        ...
        {loading ? <p className="text-xs text-slate-400 uppercase tracking-widest">Loading authorization matrix...</p> : (
            ...
            {permissions.map(permission => (
              <label key={permission.permission_code} className="flex items-center justify-between gap-4 px-4 py-3 border-b border-slate-100 text-xs">
                ...
                <select value={permission.user_override === undefined || permission.user_override === null ? 'ROLE' : permission.user_override ? 'ALLOW' : 'DENY'} onChange={event => setPermissions(current => current.map(item => item.permission_code === permission.permission_code ? { ...item, user_override: event.target.value === 'ROLE' ? null : event.target.value === 'ALLOW' } : item))} className="px-2 py-2 border border-slate-200 text-[10px] font-black">
                  <option value="ROLE">ROLE</option><option value="ALLOW">ALLOW</option><option value="DENY">DENY</option>
                </select>
              </label>
            ))}
          </div>
        )}
        <button type="button" onClick={saveOverrides} className="w-full py-3 bg-[#978C21] text-white text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2"><Save className="w-4 h-4" /> Save User Overrides</button>
      </section>
      <section className="lg:col-span-5 bg-[#FBFAF8] border border-slate-200 p-6 rounded-sm space-y-5">
        <div className="flex items-center gap-3"><History className="w-5 h-5 text-[#978C21]" /><div><h3 className="text-sm font-black uppercase tracking-widest text-slate-900">Audit Trail</h3><p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Recent identity activity.</p></div></div>
        <div className="space-y-2 max-h-[500px] overflow-auto">
          {auditLogs.length === 0 ? <p className="text-xs text-slate-400 uppercase tracking-widest">No audit events available.</p> : auditLogs.map(log => <div key={log.id} className="border border-slate-200 bg-white p-3"><p className="text-[10px] font-black uppercase tracking-widest">{log.action_code}</p><p className="text-[10px] text-slate-500">{log.entity_type} · {new Date(log.created_at).toLocaleString()}</p></div>)}
        </div>
      </section>
    </div>
  );
