...
export function buildUserImportTemplate(refData: ReferenceDataForTemplate): XLSX.WorkBook {
  ...
  const instructions = [
    ...
    { Instruction: '5. Reporting Manager', Details: 'Resolved by Employee ID only. Supports same-batch managers (row order irrelevant). Validates hierarchy: manager must be one level up, same department unless CEO (Level 1). No self-manager, no cycles. Active employees only.' },
